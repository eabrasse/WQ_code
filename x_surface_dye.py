...
dye_01 = np.zeros((NT,ny,nx))
dye_02 = np.zeros((NT,ny,nx))
Dwave = np.zeros((NT))
Hwave = np.zeros((NT))
ot = np.zeros((NT))
...
for fn in f_list:
    ...
    dye_01[old_nt:old_nt+nt,:,:] = dye_01_0[:,-1,:,:]
    dye_02[old_nt:old_nt+nt,:,:] = dye_02_0[:,-1,:,:]
...
var_list = ['dye_01','dye_02','Dwave','Hwave','ot','lon_rho','lat_rho','mask_rho']
